		...
		if (nActive ~= 0) then
			...
			if not bTargeted or EffectManager.isEffectTarget(v, rFilterActor) then
				...
				for kEffectComp, sEffectComp in ipairs(aEffectComps) do
					...
					elseif rEffectComp.type == "IFTAG" then
						-- if not checkTagConditional(rActor, v, rEffectComp.remainder) then
						break;
						-- end
...
function checkConditional(rActor, nodeEffect, aConditions, rTarget, aIgnore)
	...
	for _,v in ipairs(aConditions) do
		...
		elseif StringManager.contains(DataCommon.conditionaltags, sLower) then
			...
			-- KEL save versus tags
			-- local sTagCheck = sLower:match("^tag%s*%(([^)]+)%)$");
			if sAlignCheck then
			...
			-- elseif sTagCheck then
				-- if not checkConditionalTagHelper(rActor, sTagCheck, rTarget, aIgnore) then
					-- bReturn = false;
				-- end
...
-- KEL TAG working as IF but only for my extension needed, for reducing lag. Needed for SAVE and SIMMUNE
-- KEL Simmune
function hasTagEffect(rActor, sEffect, rEffectSpell, rTarget, bTargetedOnly, bIgnoreEffectTargets)
	if not sEffect or not rActor then
		return false;
	end
	local sLowerEffect = sEffect:lower();
	...
	for _,v in pairs(DB.getChildren(ActorManager.getCTNode(rActor), "effects")) do
		...
		if nActive ~= 0 then
			...
			for kEffectComp, sEffectComp in ipairs(aEffectComps) do
				...
				elseif rEffectComp.type == "IFTAG" then
					if not checkTagConditional(rActor, v, rEffectComp.remainder, rEffectSpell) then
						break;
					end
...
-- KEL SAVE
function getTagEffectsBonus(rActor, aEffectType, bModOnly, aFilter, rFilterActor, rEffectSpell, bTargetedOnly)
	if not rActor or not aEffectType then
		if bModOnly then
			return 0, 0;
		end
	...
	for k, v in pairs(aEffectType) do
		-- GET THE MODIFIERS FOR THIS MODIFIER TYPE
		local effbonusbytype, nEffectSubCount = getTagEffectsBonusByType(rActor, v, true, aFilter, rFilterActor, bTargetedOnly, rEffectSpell);
...
function getTagEffectsBonusByType(rActor, aEffectType, bAddEmptyBonus, aFilter, rFilterActor, bTargetedOnly, rEffectSpell)
	if not rActor or not aEffectType then
		return {}, 0;
	end
	...
	for k, v in pairs(aEffectType) do
		-- LOOK FOR EFFECTS THAT MATCH BONUSTYPE
		local aEffectsByType = getTagEffectsByType(rActor, v, aFilter, rFilterActor, bTargetedOnly, rEffectSpell);
...
function getTagEffectsByType(rActor, sEffectType, aFilter, rFilterActor, bTargetedOnly, rEffectSpell)
	if not rActor then
		return {};
	end
	local results = {};
...
function checkTagConditional(rActor, nodeEffect, aConditions, rEffectSpell, rTarget, aIgnore)
	...
	for _,v in ipairs(aConditions) do
		...
		while tagshelp[i] do
		-- if sTagCheck then
		-- if checkConditionalTagHelper(rActor, v, rTarget, aIgnore, rEffectSpell) then
...
-- function checkConditionalTagHelper(rActor, sEffect, rTarget, aIgnore, rEffectSpell)
	-- if not rActor then
		-- return false;
	-- end
