...
function applySave(rSource, rOrigin, rAction, sUser)
	...
	if rAction.sSaveResult == "autosuccess" or rAction.sSaveResult == "success" then
		...
		if rSource then
			...
			-- KEL Add partial image
			if bHalfMatch then
				TokenManager2.setSaveOverlay(ActorManager.getCTNode(rSource), -2);
			else
				TokenManager2.setSaveOverlay(ActorManager.getCTNode(rSource), -3);
			end
		...
		-- Save Overlay
		TokenManager2.setSaveOverlay(ActorManager.getCTNode(rSource), -1);
